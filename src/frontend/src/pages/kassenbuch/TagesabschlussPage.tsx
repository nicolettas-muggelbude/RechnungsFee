  ...
  return (
    <div className="text-xs border border-slate-200 rounded-lg overflow-hidden">
      {scheineZeilen.length > 0 && (
        <>
          <div className="bg-slate-50 px-3 py-1 text-slate-500 font-medium border-b border-slate-200">
            Scheine
          </div>
          {scheineZeilen.map((z) => (
            <div key={z.label} className="grid grid-cols-[1fr_40px_80px] px-3 py-1 gap-2 border-b border-slate-100">
              <span className="text-slate-700">{z.label}</span>
              <span className="text-right text-slate-500">× {z.anzahl}</span>
              <span className="text-right font-medium">{formatEuro(z.betrag)}</span>
            </div>
          ))}
        </>
      )}
      ...
      <div className="grid grid-cols-[1fr_40px_80px] px-3 py-1.5 gap-2 bg-slate-50 font-semibold text-slate-700">
    ...
    <div className="border border-slate-200 rounded-lg p-3 space-y-2">
      <div className="flex items-center justify-between gap-3">
        ...
        <button
          onClick={pruefen}
          disabled={laden}
          className="text-xs px-2.5 py-1 rounded bg-slate-100 hover:bg-slate-200 text-slate-700 border border-slate-300 disabled:opacity-50 transition-colors shrink-0"
        >
      ...
      {hash ? (
        <p className="font-mono text-[11px] text-slate-400 break-all">{hash}</p>
      ) : (
        <p className="text-xs text-amber-600 italic">Keine Signatur gespeichert (Altdaten vor GoBD-Update).</p>
      )}
...
function AbschlussDetail({ a }: { a: Tagesabschluss }) {
  ...
  return (
    <div className="px-4 pb-4 pt-3 border-t border-slate-100 space-y-4 bg-slate-50/50">
      {/* Kennzahlen */}
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 text-sm">
        <div>
          <span className="block text-xs text-slate-400 mb-0.5">Anfangsbestand</span>
          <span className="font-medium">{formatEuro(a.anfangsbestand)}</span>
        </div>
        <div>
        ...
        <div>
          <span className="block text-xs text-slate-400 mb-0.5">Buchungen gesamt</span>
          <span className="font-medium">{a.kassenbewegungen_anzahl}</span>
        </div>
      </div>
      ...
      {hatDifferenz && (
        <div className="bg-amber-50 border border-amber-200 rounded-lg px-3 py-2 text-sm">
          ...
          {a.differenz_buchungsart && (
            <span className="ml-2 text-amber-600 text-xs">· {a.differenz_buchungsart}</span>
          )}
          {a.differenz_begruendung && (
            <p className="text-amber-600 text-xs mt-0.5">{a.differenz_begruendung}</p>
          )}
        </div>
      )}
      ...
      {/* Zählprotokoll */}
      <div>
        <p className="text-xs font-semibold text-slate-600 mb-1.5">Zählprotokoll</p>
        {a.zaehlung_json ? (
      ...
      <div className="flex justify-end">
        <button
          ...
          className="text-xs px-3 py-1.5 rounded-lg border border-slate-300 bg-white hover:bg-slate-50 text-slate-700 transition-colors flex items-center gap-1.5"
        >
...
function DifferenzStatistik({ abschluesse }: { abschluesse: Tagesabschluss[] }) {
  ...
  return (
    <div className={`mb-5 rounded-xl border overflow-hidden ${
      zeigeWarnung ? 'border-amber-300' : 'border-slate-200'
    }`}>
      {/* Header */}
      <div className={`px-4 py-2.5 flex items-center justify-between ${
        zeigeWarnung ? 'bg-amber-50' : 'bg-slate-50'
      }`}>
        <span className={`text-xs font-semibold ${
          zeigeWarnung ? 'text-amber-700' : 'text-slate-600'
        }`}>
          Differenz-Statistik
        </span>
        <span className={`text-xs ${
          zeigeWarnung ? 'text-amber-600' : 'text-slate-400'
        }`}>
      ...
      {/* Kennzahlen-Kacheln */}
      <div className="bg-white grid grid-cols-2 sm:grid-cols-4 divide-x divide-y sm:divide-y-0 divide-slate-100">
        <div className="px-4 py-3">
          <span className="block text-xs text-slate-400 mb-0.5">Ausgeglichen</span>
        ...
        <div className="px-4 py-3">
          <span className="block text-xs text-slate-400 mb-0.5">Mit Differenz</span>
          <span className={`text-lg font-bold ${mitDifferenz.length > 0 ? 'text-amber-600' : 'text-slate-700'}`}>
            {mitDifferenz.length}
          </span>
          <span className="text-xs text-slate-400 ml-1">
        ...
        <div className="px-4 py-3">
          <span className="block text-xs text-slate-400 mb-0.5">Mankos</span>
          <span className={`text-lg font-bold ${mankos.length > 0 ? 'text-red-600' : 'text-slate-400'}`}>
            {mankos.length}
          </span>
          {maxManko !== null && (
        ...
        <div className="px-4 py-3">
          <span className="block text-xs text-slate-400 mb-0.5">Überschüsse</span>
          <span className={`text-lg font-bold ${ueberschuesse.length > 0 ? 'text-blue-600' : 'text-slate-400'}`}>
      ...
      {/* Ø-Differenz */}
      {mitDifferenz.length > 0 && (
        <div className="bg-white border-t border-slate-100 px-4 py-2 flex items-center gap-2">
          <span className="text-xs text-slate-400">Ø Differenz (absolut):</span>
      ...
      {/* Warnung >5% */}
      {zeigeWarnung && (
        <div className="border-t border-amber-200 bg-amber-50 px-4 py-2.5 flex items-start gap-2">
...
export function TagesabschlussPage() {
  ...
  return (
    <div className="p-6 max-w-4xl mx-auto">
      ...
      {/* Export-Leiste */}
      {abschluesse.length > 0 && (
        <div className="mb-5 flex flex-wrap items-center gap-2 bg-slate-50 border border-slate-200 rounded-xl px-4 py-3">
          <span className="text-xs font-semibold text-slate-500 mr-1">PDF-Export:</span>

          <select
            value={exportZeitraum}
            onChange={(e) => setExportZeitraum(e.target.value as 'monat' | 'jahr' | 'alle')}
            className="text-sm border border-slate-300 rounded-lg px-2 py-1.5 bg-white text-slate-700"
          >
          ...
          {exportZeitraum === 'jahr' && (
            <input
              ...
              min="2000"
              max="2099"
              className="text-sm border border-slate-300 rounded-lg px-2 py-1.5 bg-white text-slate-700 w-24"
            />
          )}
                ...
                >
                  <div className="flex items-center gap-4">
                    ...
                    <div className="flex-1 grid grid-cols-3 gap-2 text-sm">
                      <div>
                        <span className="block text-xs text-slate-400">Soll</span>
                        <span>{formatEuro(a.soll_endbestand)}</span>
                      </div>
                      <div>
                        <span className="block text-xs text-slate-400">Ist</span>
                        <span className="font-medium">{formatEuro(a.ist_endbestand)}</span>
                      </div>
                      <div>
                        <span className="block text-xs text-slate-400">Differenz</span>
                    ...
                    <div className="shrink-0 flex items-center gap-3">
                      ...
                      {a.signatur ? (
                        <span className="text-xs text-slate-300" title="Signiert">🔒</span>
                      ) : (
                        <span className="text-xs text-amber-400" title="Keine Signatur">⚠</span>
                      )}
