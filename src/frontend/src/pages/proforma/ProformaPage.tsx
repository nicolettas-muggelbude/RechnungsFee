...
function ProformaFormular({
  ...
  // Proformarechnungen, nicht beim Bearbeiten (sonst würde das Öffnen zum Editieren still
  // die Berechnungsgrundlage wechseln, siehe Issue #332)
  useEffect(() => {
    ...
    <form onSubmit={(e) => e.preventDefault()} className="space-y-5">
      ...
      <div>
        ...
        <textarea value={einleitungstext} onChange={e => setEinleitungstext(e.target.value)}
          ...
          placeholder="Erscheint vor der Positionstabelle. Leer = Standard aus den Einstellungen für Proforma-Rechnungen." />
      </div>
      ...
      <div>
        ...
        <textarea value={schlusstext} onChange={e => setSchlusstext(e.target.value)}
          rows={3} className={`${inputCls} resize-y`}
          placeholder="Erscheint nach den Positionen/Summen. Leer = Standard aus den Einstellungen für Proforma-Rechnungen." />
      </div>
  ...
  return (
    <div className="flex flex-col h-full">
      ...
      <div className="p-5 space-y-5 flex-1 overflow-y-auto">
          ...
          <div>
            ...
            <div className="border border-slate-200 dark:border-slate-700 rounded-xl overflow-hidden">
              <table className="w-full text-xs">
                ...
                <tfoot className="bg-slate-50 dark:bg-slate-900 border-t border-slate-200 dark:border-slate-700">
                  {(() => {
                    const reRabattProz = parseFloat(String(proforma.rabatt_prozent ?? '0')) || 0
                    const reRabattBetrag = proforma.rabatt_betrag ? parseFloat(String(proforma.rabatt_betrag)) : null
                    const hatRabatt = reRabattBetrag != null ? reRabattBetrag > 0 : reRabattProz > 0
                    if (!hatRabatt) return null
                    const zwischensumme = proforma.positionen.reduce((s, p) => s + (parseFloat(p.brutto as any) || 0), 0)
        ...
        {proforma.einleitungstext && (
          <div>
            <p className="text-xs font-semibold text-slate-500 dark:text-slate-400 uppercase tracking-wide mb-1">Einleitungstext</p>
            <p className="text-sm text-slate-600 dark:text-slate-300 bg-slate-50 dark:bg-slate-900 rounded-lg px-3 py-2 whitespace-pre-wrap">{proforma.einleitungstext}</p>
          </div>
        )}

        {proforma.schlusstext && (
          <div>
            <p className="text-xs font-semibold text-slate-500 dark:text-slate-400 uppercase tracking-wide mb-1">Schlusstext</p>
            <p className="text-sm text-slate-600 dark:text-slate-300 bg-slate-50 dark:bg-slate-900 rounded-lg px-3 py-2 whitespace-pre-wrap">{proforma.schlusstext}</p>
          </div>
        )}
...
export function ProformaPage() {
  ...
  const [splitterBreite, startSplitterDrag] = useSplitterBreite('proforma', 33)
