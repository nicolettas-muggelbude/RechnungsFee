...
- **Profilmanager – mehrere Firmen pro Installation (natürliche Personen, z. B. freiberuflich + gewerblich)** – eine Installation, mehrere Profile statt einer festen `APP_DATA_DIR`/`rechnungsfee.db` ([connection.py:9-14](../src/backend/database/connection.py#L9-L14)): jedes Profil bekommt einen eigenen Datenordner (`profile/<name>/` – eigene DB, eigene Belege, eigenes Backup, eigene GoBD-Trigger). Ein zentraler Profil-Zeiger außerhalb der Profile speichert das aktive Profil; Wechsel = Zeiger umschreiben + Neustart (gleiches Relaunch-Muster wie beim Backup-Restore). Setup-Wizard wiederverwendbar für „neues Profil anlegen". Bereits verworfener Zwischen-Ansatz: RechnungsFee einfach zweimal installieren (wie bei den alten `testing/*`-Branches per eigenem `productName`/`identifier` in `tauri.conf.json`) – funktioniert nicht, weil `APP_DATA_DIR` im Backend fest auf den String `"RechnungsFee"` verdrahtet ist und unabhängig vom Tauri-Identifier bleibt; zwei Installationen würden dieselbe DB teilen (WAL-Konflikte/Datenkorruption).
  **Pflicht bei Umsetzung:** Bestandsinstallationen müssen beim ersten Start automatisch in ein Standard-Profil migriert werden (bestehende flache `rechnungsfee.db`/`uploads/` → `profile/Standard/`) – sonst wirkt es für heutige Nutzer wie Datenverlust.
  EÜR/Anlagenverzeichnis/GoBD bleiben sauber pro Profil getrennt. Bewusst **keine** automatische Zusammenführung von Umsätzen über mehrere Profile für die §19-Kleinunternehmer-Schwelle/UStVA/§141 AO (die für die Person als Steuersubjekt insgesamt gelten) – wer mehrere Firmen führt, kennt diese Pflicht und prüft/meldet das selbst bzw. mit Steuerberater. Diskussion 2026-07.
