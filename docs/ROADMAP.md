...
- **Profilmanager optional aktivierbar** (Folge-Feature zu v0.6.0) – der Menüpunkt „Profile" ([ProfilePage.tsx](../src/frontend/src/pages/profile/ProfilePage.tsx), Nav-Link in `AppLayout.tsx`) ist aktuell immer sichtbar, auch für die meisten Nutzer die nur ein einziges Profil je brauchen werden – unnötiges Rauschen im Menü. Soll standardmäßig ausgeblendet und über einen Schalter aktivierbar sein.
  **Wichtiger Unterschied zu allen anderen `xxx_aktiv`-Flags** (`wiederkehrend_aktiv`, `bank_import_aktiv`, `angebote_aktiv` usw., siehe [models.py](../src/backend/database/models.py) `Unternehmen`): diese liegen alle in der Unternehmen-Tabelle der jeweils aktiven Profil-DB – für den Profilmanager selbst ist das ungeeignet, weil das Flag dann pro Profil einzeln gesetzt werden müsste und ein Profil, in dem es (versehentlich) deaktiviert ist, keinen Weg mehr zurück zum Profil-Wechsel böte. Der Schalter muss stattdessen **installationsweit außerhalb jedes Profils** liegen (z. B. als zusätzliches Feld direkt neben dem Zeiger in `profile.json`, oder eine eigene Datei daneben in `BASE_DIR`) – analog dazu, wie der Profil-Zeiger selbst schon außerhalb der Profile lebt.
  Der Nav-Link muss außerdem **immer sichtbar bleiben, sobald bereits mehr als ein Profil existiert** – sonst könnte man nach dem Anlegen eines zweiten Profils nicht mehr zwischen ihnen wechseln, falls der Schalter zwischenzeitlich deaktiviert wurde.
